...
import com.wy.model.Info;
import com.wy.model.PhotoInfo;
...
public class UserInfoAction extends ActionSupport implements
		ModelDriven<UserInfo>, ServletRequestAware {
	...
	public String friend_delete() {
		...
		if (friendsTypes != null) {
			...
			for (String type : friendsTypes) {
				date_types = com.wy.tools.ValidateExpression.deleteString(
						date_types, type);
			}
	...
	public String phtType_delete() {
		...
		if (phtTypes != null) {
			...
			for (String phtType : phtTypes) {
				date_phtType = com.wy.tools.ValidateExpression.deleteString(
	...
	public String artType_update() {
		...
		if (flag) {
			if (!newsType.equals("")) {
				date_artType = com.wy.tools.ValidateExpression.deleteString(
	...
	public String artType_delete() {
		...
		if (artTypes != null) {
			...
			for (String artType : artTypes) {
				date_artType = com.wy.tools.ValidateExpression.deleteString(
						date_artType, artType);

				hql = "from ArticleInfo where author = '" + account
						+ "' and typeName = '" + artType + "'";
				ObjectDao<ArticleInfo> dao = new ObjectDao<ArticleInfo>();
				List<ArticleInfo> list = dao.queryList(hql);
				for (ArticleInfo articleInfo : list) {
	...
	public void validateUserPassword_update() {
		...
		oldPassword = com.wy.tools.ValidateExpression.encodeMD5(oldPassword);
		String account = (String) request.getSession().getAttribute("account");
		objectDao = new ObjectDao<UserInfo>();
		hql = "from UserInfo where account = '" + account + "'";
		userInfo = objectDao.queryFrom(hql);
	...
	public String userPassword_update() {
		String oldPassword = com.wy.tools.ValidateExpression.encodeMD5(request.getParameter("password"));
	...
	public void validateUser_update() {
		...
		if (userInfo.getEmail().equals("")) {
			...
			if (!com.wy.tools.ValidateExpression
	...
	public void validateCheckUser() {
		...
		if (null == user) {
			...
			String password = com.wy.tools.ValidateExpression
	...
	public String updateCommendUser() {
		...
		if (userInfo.getFreeze().equals("解冻")) {
			if (userInfo.getCommend().equals("是")) {
	...
	public String addUserInfo() {
		objectDao = new ObjectDao<UserInfo>();
		userInfo.setPassword(com.wy.tools.ValidateExpression.encodeMD5(userInfo
				.getPassword()));
		userInfo.setHomepage(userInfo.getHomepage() + userInfo.getAccount());
		boolean flag = objectDao.saveT(userInfo);
		String result = "";
		if (flag) {
			String descPath = ServletActionContext.getRequest().getRealPath(
					"/" + userInfo.getAccount() + "");
			String sourPath = ServletActionContext.getRequest().getRealPath(
					"/templet/index.jsp");
			if (com.wy.tools.FileOperation.buildJSP(sourPath, descPath,
					userInfo.getAccount())) {
				result = "您注册成功！";

				request.getSession().setAttribute("freeze",
						userInfo.getFreeze());
